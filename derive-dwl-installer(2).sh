...
# dérive doesn't ship doas by default. If we're already root (e.g. a
# fresh VM/install logged in as root), just skip it entirely so the
# script can run start to finish without opendoas at all.
if [ "$(id -u)" -eq 0 ]; then
	doas() { "$@"; }
fi

# A handful of ports on this tree reference build-time variables
# (PREFIX, CPPFLAGS) at the top level of ndmake.sh before libdmake.sh
# ever gets sourced to give them a default -- under `set -u` that's a
# hard error. Pre-exporting them empty here fixes every port that hits
# this, not just the ones discovered tonight (freetype, samurai).
# Some autotools-based ports (python3's configure being the one that
# actually bit us) auto-detect a compiler by searching PATH for `gcc`
# specifically, and on this system that finds something that exists
# but can't produce working executables -- while `cc` (clang under the
# hood) works fine everywhere else. Force CC up front so configure
# scripts never go looking for gcc on their own.
export CC="${CC:-cc}"
...
# (defined early since the doas bootstrap step below needs it)
# ---------------------------------------------------------------------
build_port() {
	dir="$1"
	echo "==> building $dir"
...
# If doas isn't already present (fresh non-root setup) build it now,
# before anything else needs it. Skipped automatically when already
# root, since the shim above makes doas a no-op passthrough anyway.
if ! command -v doas > /dev/null 2>&1 && [ "$(id -u)" -ne 0 ]; then
	echo "==> bootstrapping doas (opendoas)"
	build_port devel/samurai
	build_port devel/shinobi
	build_port devel/byacc
	build_port userspace/opendoas
fi
